...
First, edit the `linemake.f` file (line 38) at the start of the program, to point the code to its species linelists `linepath='/path/to/linemake/mooglists'`. Then, compile the code:
...
`Co II`| [Lawler et al. (2018, ApJS, 238, 7)](https://ui.adsabs.harvard.edu/abs/2018ApJS..238....7L); there are 12 lines in this paper  with good lab hfs patterns. To these we have added another 4 lines with new *gf* values but approximate HFS patters from the `Co I` paper; these appear with the notations `LAW???` in a linelist generated by `linemake`
